...
public class Month_sparepart_Controller {
	...
	public void excelExport(HttpServletResponse response, String store_id, OrgType store_type, String date_start, String date_end) throws IOException, ParseException {
			...
			if (equipmentType.getSubtypes() != null) {
				...
				for (Month_sparepart_subtype equipmentSubtype : equipmentType.getSubtypes()) {
					...
					for (Month_sparepart_prod prod:prods) {
						...
						if (store_type == OrgType.store_prepare) {
							// 额定数量
	...
	private void sparepart_addRow1(XSSFWorkbook wb, Sheet sheet,OrgType store_type) {
	...
	public void excelTpl(HttpServletResponse response,OrgType store_type) throws IOException {
