...
public class Day_sparepart_Service extends AbstractService<Day_sparepart, Day_sparepart_PK> {
	 ...
	 */
	public void proc_report_day_sparepart(OrgType orgType){	
		String day_in=yyyyMMdd_format.format(new Date());
		//查询所有可以编辑的备品备件查库
		List<Org> stores=orgService.queryStores4Combo(true,true,orgType);
		for(Org store:stores){
	...
	public List<Day_sparepart_type> queryDay_sparepart(String store_id,OrgType orgType,String day_start,String day_end){
		if(!StringUtils.hasText(store_id) && orgType!=null){
			List<Org> stores=orgService.queryStores4Combo(true,true,orgType);
	...
	public List<Month_sparepart_type> queryMonth_sparepartVO(String store_id,OrgType orgType,String day_start,String day_end){
	 ...
	 * @return
	 */
	public Map<String,Object> queryMonth_yesterdaynum(String store_id,OrgType orgType,String day_start) {
