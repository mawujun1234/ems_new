...
//		}
	...
	public  EquipmentTypeAbstract create(@RequestBody EquipmentTypeAbstract equipmentType) {
		...
		if(!StringUtils.hasText(equipmentType.getParent_id())){
			...
			int count=equipmentTypeService.queryCount(Cnd.select().andEquals(M.EquipmentType.id, equipmentType.getId()));
			if(count>0){
				throw new BusinessException("编码已经存在");
			}
			equipmentTypeService.create(BeanUtils.copyOrCast(equipmentType, EquipmentType.class));
		} else{
			int count=equipmentSubtypeService.queryCount(Cnd.select().andEquals(M.EquipmentSubtype.id, equipmentType.getId()));
			if(count>0){
				throw new BusinessException("编码已经存在");
			}
	...
	public boolean have_same_style(String subtype_id,String style){
		...
		int count=equipmentProdService.queryCount(Cnd.count().andEquals(M.EquipmentProd.subtype_id, subtype_id).andEquals(M.EquipmentProd.style, style));
		if(count>0){
			return true;
		} else {
			return false;
	...
	public  EquipmentProd createProd(@RequestBody EquipmentProd equipmentProd) {
		...
		int count=equipmentProdService.queryCount(Cnd.select().andEquals(M.EquipmentProd.id, equipmentProd.getId()));
		if(count>0){
			throw new BusinessException("编码已经存在");
		}
