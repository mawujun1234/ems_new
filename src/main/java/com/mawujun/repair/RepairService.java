...
//		newRepair(repairs);
	...
	public Pager<RepairVO> storeMgrQuery(Pager<RepairVO> page){
		...
		Pager<RepairVO> results=repairRepository.storeMgrQuery(page);
		List<RepairVO> list=results.getRoot();
		for(RepairVO repairVO:list){
			if(orgService.get(repairVO.getStr_out_id())!=null){
				repairVO.setStr_out_name(orgService.get(repairVO.getStr_out_id()).getName());
			}
			if(orgService.get(repairVO.getRpa_id())!=null){
				repairVO.setRpa_name(orgService.get(repairVO.getRpa_id()).getName());
			}
			if(orgService.get(repairVO.getStr_in_id())!=null){
				repairVO.setStr_in_name(orgService.get(repairVO.getStr_in_id()).getName());
			}
	...
	public Pager<RepairVO> repairInQuery(Pager<RepairVO> page){
		//List<Store> stores=storeRepository.queryAll();
		Pager<RepairVO> results=repairRepository.repairInQuery(page);
		List<RepairVO> list=results.getRoot();
		for(RepairVO repairVO:list){
...
//			}
	...
	public Pager<RepairVO> repairMgrQuery(Pager<RepairVO> page){
		//List<Store> stores=storeRepository.queryAll();
		Pager<RepairVO> results=repairRepository.repairMgrQuery(page);
		List<RepairVO> list=results.getRoot();
		for(RepairVO repairVO:list){
//			for(Store store:stores){
//				if(store.getId().equals(repairVO.getStr_out_id())){
