Ext.define('Ems.baseinfo.WorkunitPoleGrid',{
	extend:'Ext.grid.Panel',
	requires: [
	     'Ems.baseinfo.Pole',
	     'Ems.baseinfo.WorkunitSelPoleWindow'
	],
	columnLines :true,
	stripeRows:true,
	viewConfig:{
	...
	initComponent: function () {
	  ...
	  me.store=Ext.create('Ext.data.Store',{
			...
			proxy: {
			        type: 'ajax',
			        url: Ext.ContextPath+'/pole/queryPolesByWorkunit.do',
			        reader: {
			        	type:'json',
			            rootProperty: 'root'
			        }
     	...
     	var reload = new Ext.Action({
		    ...
		    handler: function(){
		    	...
		    	me.getStore().getProxy().extraParams=Ext.apply(me.getStore().getProxy().extraParams,{
		    		...
		    		nameORcode:nameORcode.getValue(),
...
//		};
    ...
    onCreate:function(){
		...
		var win=Ext.create('Ems.baseinfo.WorkunitSelPoleWindow',{
			width:800,
			workunit_id:me.getStore().getProxy().extraParams.workunit_id,
			...
			buttons:[{
				...
				handler:function(){
					...
					for(var i=0;i<records.length;i++){
					 ...
					 Ext.Ajax.request({
					        url : Ext.ContextPath+'/pole/addWorkunit.do',
					        method : 'post',
    ...
    onDelete:function(){
		...
		Ext.Msg.confirm("删除",'确定要删除吗?', function(btn, text){
				if (btn == 'yes'){
					...
					Ext.Ajax.request({
						url:Ext.ContextPath+'/pole/removeWorkunit.do',
						params:{
							workunit_id:me.getStore().getProxy().extraParams.workunit_id,
							pole_ids:pole_ids
						},
